...
CREATE TABLE IF NOT EXISTS chess_tournaments (
    tournamentKey  INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    winnerKey      INTEGER,
    UNIQUE(tournamentID),
    FOREIGN KEY (winnerKey) REFERENCES chess_users(userKey)
...
CREATE TABLE IF NOT EXISTS chess_games (
    ...
    FOREIGN KEY (tournamentKey) REFERENCES chess_tournaments(tournamentKey)
        ON UPDATE CASCADE ON DELETE SET NULL,
    FOREIGN KEY (whiteKey) REFERENCES chess_users(userKey)
        ON UPDATE CASCADE ON DELETE SET NULL,
    FOREIGN KEY (blackKey) REFERENCES chess_users(userKey)
        ON UPDATE CASCADE ON DELETE SET NULL
);
""")
...
CREATE TABLE IF NOT EXISTS chess_moves (
    moveKey     INTEGER PRIMARY KEY AUTOINCREMENT,
    move        TEXT NOT NULL,
    madeBy      TEXT NOT NULL,
    evalAfter   REAL,
    elapsedTime INTEGER,
    gameKey     INTEGER NOT NULL,
    FOREIGN KEY (gameKey) REFERENCES chess_games(gameKey)
...
CREATE TABLE IF NOT EXISTS tournamentEntry (
    entryKey      INTEGER PRIMARY KEY AUTOINCREMENT,
...
# (Optional) seed data – same values as before, just reuse your INSERTs
# You can copy your existing INSERT INTO ... statements from MySQL
# and run them here; sqlite will accept them as long as you remove backticks.
